...
This project dogfoods arc. Every implementation change must be traceable to a requirement or decision. Graph capture is delegated to the **arc-requirements-engineer** agent — the remedy for a missing R-xxx is to hand the request to that agent, not to `appeal:`.
...
When modifying `.arc/`, run `arc check` and confirm it reports no errors. `arc-check-passed` in the commit message asserts the check passed.
...
Passive instruction does not change agent behavior during focused coding — an agent told to dogfood will still implement features without recording architecture mid-task. The validator is the one signal that fires at the commit boundary. The semantic check (a cited ID must relate to the work, §2) and constrained appeals (§3) keep enforcement honest: it is only as leaky as the escape hatch is narrow.
